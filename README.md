# pyMAG
Modulo per la scrittura e controllo di file xml MAG - Metadati Amministrativi e Gestionali usando il linguaggio di programmazione Python. 


## Installazione 
L'installazione può essere fatta usando `pip`:

    pip install git+https://github.com/giacomomarchioro/pyMAG


## Esempio di utilizzo 
Il modulo mappa la struttura dello standard MAG ad un oggetto. Il modo più semplice di utilizzarlo è modificare gli esempi forniti nell'omonima cartella.
...
rawgrp.image_metrics.set_bitpersample('16,16,16') # questo bit per sample non sarebbe valido
...
rawgrp.format.set_name('TIF')
rawgrp.format.set_compression('Uncompressed')
rawgrp.format.set_mime('image/tiff')
...
rawgrp.scanning.set_scanner_manufacturer("Nikkon")
rawgrp.scanning.set_scanner_model("D810")
